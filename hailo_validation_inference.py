...
Refined Hailo-8 inference pipeline
...
class HailoApp:
    ...
    def inference_callback(self, infer_results=None, bindings_list=None, validate: bool = True, **kwargs) -> None:
        ...
        for info in bindings_list:
            ...
            # # Append local for batch

            # if validate:

            #     true_label = self._extract_true_from_meta(meta)
        ...
        if validate and batch_truths:
            ...
            logging.info("Batch processed: number=%d batch_accuracy=%.2f%%", len(truths_arr), batch_accuracy * 100.0)
